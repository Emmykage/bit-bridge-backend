...
module Api
  module V1
    class UsersController < ApplicationController
        ...
        if current_user.valid_password?(user_params[:old_password])
          if user_params[:password] == user_params[:confirm_password]
            if current_user.update(user_params)
              ...
              render json: { message: "failed to update password:  #{current_user.errors.full_messages.to_sentence}" },
      ...
      def user_params
        params.require(:user).permit(:email, :active, :password, :old_password, :confirm_password, :password_token,
                                     user_profile_attributes: %i[id first_name last_name phone_number])
      end
